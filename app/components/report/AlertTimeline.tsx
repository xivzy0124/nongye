...
export default function AlertTimeline({ alerts }: { alerts: AlertItem[] }) {
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto scrollbar-hide space-y-1.5 pr-1">
        {alerts.map((alert, idx) => {
          const level = levelMap[alert.level];
          return (
            <div
              key={alert.id}
              className={`relative rounded-lg ${level.bg} border ${level.border} p-2 hover:brightness-110 transition-all cursor-pointer group number-roll`}
            ...
            >
              ...
              <div className="relative z-10 flex gap-2">
                <div className="flex flex-col items-center">
                  <span className="text-xs scale-pulse" style={{ animationDelay: `${idx * 0.25}s` }}>{level.icon}</span>
                  <div className="w-px flex-1 bg-gradient-to-b from-cyan-500/30 to-transparent mt-0.5"></div>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-0.5">
                    <span className={`text-[10px] font-bold ${level.text}`}>{alert.title}</span>
                    <span className="text-[9px] text-gray-500 font-mono">{alert.time}</span>
                  </div>
                  <p className="text-[9px] text-gray-300 leading-snug">{alert.description}</p>
                </div>
              </div>
            </div>
          );
